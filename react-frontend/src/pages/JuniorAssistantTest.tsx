...
import './JuniorAssistantTest.css';

const JuniorAssistantTest: React.FC = () => {
  ...
  const fetchPassages = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/passages/test/Junior%20Assistant');
  ...
  const generateFeedback = (results: ReturnType<typeof calculateResults>) => {
    ...
    if (accuracy < 80) {
      feedbackText += ' Accuracy below 80%! You need to improve to qualify for Junior Assistant.';
    }
