...
const SSCCHSLTest: React.FC = () => {
  ...
  const fetchPassages = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/passages/test/SSC%20CHSL');
  ...
  const generateFeedback = (results: ReturnType<typeof calculateResults>) => {
    ...
    if (accuracy >= 70) {
      if (grossSpeed === 0) {
        ...
        feedbackText += ' Achhi progress hai, lekin 35 WPM tak pahunchne ke liye lagataar typing karo!';
      } else if (grossSpeed <= 34) {
        feedbackText += ' Bas thoda aur push karo, 35 WPM bilkul paas hai!';
    ...
    if (accuracy < 80) {
      feedbackText += ' Accuracy below 80%! You need to improve to qualify for SSC CHSL.';
    }
